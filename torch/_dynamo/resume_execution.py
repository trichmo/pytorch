...
def _unwrap_resume_arg_template(dummy: Any) -> None:
    # NOTE: __unwrap_for_resume is installed as a global (not imported here --
    # a local import would need a fresh local slot, which the code-object
    # assembler can't resolve when this template is spliced into an arbitrary
    # target) by InstructionTranslatorBase.create_resume in symbolic_convert.py.
    global __unwrap_for_resume
    dummy = __unwrap_for_resume(dummy)  # type: ignore[name-defined]
